...
export async function api<T>(
  ...
  if (!response.ok || !body.success)
    throw new Error(body.message ?? 'ไม่สามารถเชื่อมต่อระบบได้');
  return body.data;
}
...
export const getDailySalesReport = (date?: string) =>
  api<DailySalesReport>(
    `/reports/daily-sales${date ? `?date=${encodeURIComponent(date)}` : ''}`,
  );
export const createPOSOrder = (
  ...
  api<{ id: number; total: number }>('/pos/orders', {
    method: 'POST',
    body: JSON.stringify({ channel, items }),
  });
...
export const createInventory = (item: Omit<InventoryItem, 'id' | 'status'>) =>
  api<{ id: number }>('/inventory', {
    method: 'POST',
...
export const createStockRequest = (
  ...
  api<{ id: number; status: string }>('/stock-requests', {
    method: 'POST',
