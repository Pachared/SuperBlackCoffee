...
type MenuItem = { id: string; name: string; category: string; storePrice: number; linemanPrice: number; status: 'available' | 'soldout'; position: string };
...
export function PosOverviewPage() {
  ...
  useEffect(() => {
    ...
    listMenuItems().then((items) => {
      if (!active) return;
      setMenu(items.map((item, index) => ({ id: String(item.id), name: item.name, category: posCategory(item.name, item.category), storePrice: item.storePrice, linemanPrice: item.linemanPrice, status: item.status, position: `${12 + ((index * 21) % 76)}% ${24 + ((index * 17) % 64)}%` })));
