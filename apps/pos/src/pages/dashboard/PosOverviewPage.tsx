...
function posCategory(name: string, category: string) {
  ...
  if (name.includes('ร้อน'))
    return category.includes('ชา') ? 'เมนูชาร้อน' : 'เมนูร้อน';
...
export function PosOverviewPage() {
  ...
  const [salesChannel, setSalesChannel] = useState<'หน้าร้าน' | 'LINE MAN'>(
    'หน้าร้าน',
  );
  ...
  const visibleMenu = useMemo(
    ...
    [category, deferredQuery, menu],
  );
  const cartItems = menu.filter((item) => cart[item.id]);
  const priceFor = (item: MenuItem) =>
    salesChannel === 'LINE MAN' ? item.linemanPrice : item.storePrice;
  const total = cartItems.reduce(
    (sum, item) => sum + priceFor(item) * cart[item.id],
    0,
  );
  useEffect(() => {
    let active = true;
      ...
      .then((items) => {
        ...
        setMenu(
          items.map((item, index) => ({
            id: String(item.id),
            name: item.name,
            category: posCategory(item.name, item.category),
  ...
  const submitOrder = async () => {
    try {
      ...
      await createPOSOrder(
        salesChannel === 'หน้าร้าน' ? 'storefront' : 'lineman',
        cartItems.map((item) => ({
        ...
        <Box>
          ...
          >
            <Box>
              ...
              >
                เลือกราคาให้ตรงกับช่องทางก่อนรับออเดอร์
              </Typography>
            </Box>
            <Box
              sx={{
            ...
            >
              {(['หน้าร้าน', 'LINE MAN'] as const).map((channel) => {
                const selected = salesChannel === channel;
                const lineMan = channel === 'LINE MAN';
                return (
                  <Button
                    key={channel}
                    ...
                    sx={{
                      ...
                      boxShadow: 'none',
                      '&:hover': {
                        borderColor: lineMan ? '#06C755' : '#201914',
                        bgcolor: selected
                          ? lineMan
                            ? '#05B64F'
                            : '#3c2d24'
              ...
              {categories.map((item) => (
                <Button
                  ...
                  sx={{
                    ...
                    borderRadius: '12px',
                    borderColor: '#d8c8bd',
                    bgcolor: category === item ? '#201914' : '#fff',
                    color: category === item ? '#fff' : '#5f4b3d',
                    fontFamily: 'Kanit, sans-serif',
                    fontSize: 12,
                    boxShadow: 'none',
                  }}
        ...
        >
          ...
          >
            <Box>
              ...
              >
                {salesChannel === 'หน้าร้าน'
                  ? 'ขายหน้าร้าน'
                  : 'ขายผ่าน LINE MAN'}
              </Typography>
            </Box>
            <Chip
              label={`${cartItems.length} รายการ`}
              size="small"
              sx={{
                height: 25,
                borderRadius: '10px',
          ...
          >
            {cartItems.length === 0 ? (
              ...
              >
                ...
                >
                  เลือกเมนูจากด้านซ้ายเพื่อเริ่มออเดอร์
                </Typography>
              </Box>
            ) : (
              <Box>
