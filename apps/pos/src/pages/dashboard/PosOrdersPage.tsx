...
type SupplyOrder = {
  id: string;
  date: string;
  status: 'จัดส่งแล้ว' | 'กำลังจัดเตรียม' | 'รับเข้าแล้ว';
...
export function PosOrdersPage() {
  ...
  useEffect(() => {
    ...
    listStockRequests()
      .then((requests) => {
        if (!active) return;
        const statusMap: Record<string, SupplyOrder['status']> = {
          pending: 'กำลังจัดเตรียม',
          approved: 'กำลังจัดเตรียม',
          preparing: 'กำลังจัดเตรียม',
          completed: 'รับเข้าแล้ว',
        };
        setSupplyOrders(
          requests.map((request) => ({
            id: `REQ-${request.id}`,
            date: formatDate(request.createdAt),
            status: statusMap[request.status] ?? 'จัดส่งแล้ว',
