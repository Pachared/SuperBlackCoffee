...
func (r *postgresMenuRepository) List(ctx context.Context, branchID int64) ([]model.MenuItem, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Name, &item.Category, &item.StorePrice, &item.LinemanPrice, &item.CostPrice, &item.Status, &item.ImageURL, &ingredients); err != nil {
			return nil, err
		}
